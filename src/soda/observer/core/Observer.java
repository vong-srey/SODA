	...
	public void run(){
		while(!stopObserving.get()){
			try {
				...
				// because this process requires connecting to elasticsearch
				// server which can be time consuming
				// so, it can effect to next cycle of the observer of we dt hv a
				// seperate thread to handle this.
				...
				if (query.trim().isEmpty() || query == null || email == null
						|| email.trim().isEmpty()) {
				...
				String rsltStr = null;
				List<List<Map<String, String>>> results = queryProc
						.processOrQueriesOfAndQueries(query, obsvrFrqc);
				if (!results.isEmpty()) {
					rsltStr = queryProc.convertOrRsltOfAndRsltToString(results);
				} else {
					appLogger.info("System is observing this query: " + query
							+ " But, there's no result found.");
				}
				...
				// every notifier is sending an alert message to the registered
				// contact
				if (!(rsltStr == null || rsltStr.trim().isEmpty())) {
					if (debug) {
						System.out.println("sent out mail: \n" + rsltStr);
					...
					try {
						sendEmailFrqncy = Integer.parseInt(ConfigReader
								.getProperty("ResendEmailFrequency"));
					} catch (Exception e) {
						appLogger
								.error("ResendEmailFrequency property is incorrectly configured");
					}
					...
					if (debug)
						System.out.println("There's nothing has been observed.");
				}
